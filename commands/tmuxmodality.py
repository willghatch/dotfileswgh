...
#----------------------------------------------------------------------------
#################################################
# I got this from https://github.com/mtl/tmux-modality/
# and have modified it from there...
...
def mode_default():
    ...
    # Ok, my versions here
    binder.bind( "C-t", [ "send-prefix" ], use_prefix = True )
    #binder.bind( "t", [ "send-prefix" ], use_prefix = True )
    binder.bind( "v", [ "split-window", "-h" ], use_prefix = True )
    binder.bind( "s", [ "split-window", "-v" ], use_prefix = True )
    binder.bind( "f", [ "break-pane" ], use_prefix = True ) # send pane to it's own window
    binder.bind( "d", [ "detach-client" ], use_prefix = True )
    binder.bind( "q", [ "detach-client" ], use_prefix = True )
#
    # L1
    binder.bind( "j", [ "select-pane", "-t", ":.+" ], use_prefix = True )
    binder.bind( "k", [ "select-pane", "-t", ":.-" ], use_prefix = True )
    binder.bind( "J", [ "swap-pane", "-U" ], use_prefix = True ) # swap with previous
    binder.bind( "K", [ "swap-pane", "-D" ], use_prefix = True ) # swap with next
    binder.bind( "h", [ "resize-pane", "-L", "5" ], use_prefix = True ) # Needs repeat (-r)
...
class Binder( object ):
    ...
    def _emit_bindings( self ):
        ...
        #unbind EVERYTHING to not have crap hanging out
        for key in list(self.no_prefix_single_keys) + self.no_prefix_special_keys:
            self._emit_binding( Binding(key, use_prefix=False), unbind = True )
            self._emit_binding( Binding(key, use_prefix=True), unbind = True )
        # Emit bindings:
