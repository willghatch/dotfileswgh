...
#
    binder.bind( "b", [ "split-window", "-v", "zsh -i -t", " \\; ", "send-keys", "tmux switch-client -t " ], use_prefix = True )
    binder.bind( "C", [ "command-prompt", "-p", "window name: ", "new-window -n %%" ], use_prefix = True )
...
def mode_insert():
    ...
    binder.bind( "h", [ "run-shell", python + ' ' +
